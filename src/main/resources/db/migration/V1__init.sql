CREATE TABLE credit_applications (
    ...
    CONSTRAINT chk_status CHECK (status IN
        ('CREATED', 'UNDER_REVIEW', 'APPROVED', 'REJECTED', 'CANCELLED'))
);

CREATE INDEX idx_credit_applications_status
    ON credit_applications (status);

CREATE INDEX idx_credit_applications_rfc
    ON credit_applications (customer_rfc);

CREATE INDEX idx_credit_applications_created_at
